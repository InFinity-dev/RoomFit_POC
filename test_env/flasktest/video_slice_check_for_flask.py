...
def run(video_name):
    ...
    for elem in below_mean_frames_list:
        ...
        print(f'{pose_count}번째 포즈 프레임 구간 : {elem} -> {min_start}분 {sec_start}초 ~ {min_end}분 {sec_end}초 : {duration_min}분 {duration_sec}초')
        pose_count += 1
    ...
    for start, end, dur in vid_slice_info:
        ...
        print(pose_num, middle_frame, dur)
        cap.set(cv2.CAP_PROP_POS_FRAMES, middle_frame - 1)
        res, img = cap.read()
        img = cv2.resize(img, (600, 400))
        cv2.imwrite(f'{save_path}/pose_{pose_num}.jpg', img)
